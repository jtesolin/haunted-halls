...
This workspace contains two repositories: `haunted-halls` (this one) and `haunted-halls-engine`. Unless the user explicitly asks for coordinated cross-repository work, operate only in `haunted-halls`. Do not make mirrored or speculative changes in `haunted-halls-engine` just because it is open in the same workspace.
