...
1. Verify ownership of `tesolin.us` with Google.
2. Set `frontend_custom_domain` locally in the ignored Terraform variables file when the domain is ready.
3. Terraform creates the Cloud Run frontend domain mapping.
4. Read the DNS records generated by Cloud Run for the domain mapping.
5. Add those records at the external DNS provider.
6. Wait for DNS propagation and Google-managed HTTPS certificate issuance.
7. Verify `https://haunted-halls.tesolin.us/api/health`.
8. Update the Google OAuth origin and callback for the custom domain.
9. In a later D6C step, switch `NEXTAUTH_URL` to the custom domain.
10. Perform browser authentication and gameplay verification.
...
These are operator steps outside the D6A Terraform code path. No interactive Search Console verification is included in this task.

### DNS is manual and authoritative

Cloud Run will return the required DNS records only after the domain mapping exists. D6B will retrieve them with an equivalent of:
...
The exact returned records are authoritative. D6A does not hardcode guessed DNS values or create Cloud DNS resources. The external DNS provider continues to own the `tesolin.us` records.
